...
#    >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>    
    def has_cannot_place_alerts(self):
        """
        Returns whether this job has currently active 'Cannot_Place_Alert' alerts.

        :returns: *True* if the monitored job has active active 'Cannot_Place_Alert'.
        :rtype: bool
        """
        sql = "SELECT id FROM alerts \
        WHERE job_id = {} \
        AND active = 1 AND type = 'Cannot_Place_Alert'".format(self.job_id)
        self.cursor.execute(sql)
        if not self.cursor.fetchall():
            return False
        return True
#    >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>    
    def has_Damaged_Item_alerts(self):
        """
        Returns whether this job has currently active 'Damaged_Item' alerts.

        :returns: *True* if the monitored job has active active 'Damaged_Item'.
        :rtype: bool
        """
        sql = "SELECT id FROM alerts \
        WHERE job_id = {} \
        AND active = 1 AND type = 'Damaged_Item'".format(self.job_id)
        self.cursor.execute(sql)
        if not self.cursor.fetchall():
            return False
        return True
